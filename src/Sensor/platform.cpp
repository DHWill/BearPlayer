...
//	p_platform->address = 0x49;
	p_platform->fd = open("/dev/i2c-4", O_RDONLY);
	if (p_platform->fd == -1) {
		LOG("Failed to open /dev/i2c-4\n");
...
int32_t write_read_multi(int fd, uint16_t i2c_address, uint16_t reg_address,
		uint8_t *pdata, uint32_t count, int write_not_read) {
	...
	if (write_not_read) {
		...
		do {
			data_size =
					(count - position) > VL53L5CX_COMMS_CHUNK_SIZE ?
							VL53L5CX_COMMS_CHUNK_SIZE : (count - position);
...
int32_t write_multi(int fd, uint16_t i2c_address, uint16_t reg_address,
		uint8_t *pdata, uint32_t count) {
	return (write_read_multi(fd, i2c_address, reg_address, pdata, count, 1));
}

int32_t read_multi(int fd, uint16_t i2c_address, uint16_t reg_address,
		uint8_t *pdata, uint32_t count) {
	return (write_read_multi(fd, i2c_address, reg_address, pdata, count, 0));
}

uint8_t RdByte(VL53L5CX_Platform *p_platform, uint16_t reg_address,
		uint8_t *p_value) {
	return (read_multi(p_platform->fd, p_platform->address, reg_address,
			p_value, 1));
}

uint8_t WrByte(VL53L5CX_Platform *p_platform, uint16_t reg_address,
		uint8_t value) {
	return (write_multi(p_platform->fd, p_platform->address, reg_address,
			&value, 1));
}

uint8_t RdMulti(VL53L5CX_Platform *p_platform, uint16_t reg_address,
		uint8_t *p_values, uint32_t size) {
	return (read_multi(p_platform->fd, p_platform->address, reg_address,
			p_values, size));
}

uint8_t WrMulti(VL53L5CX_Platform *p_platform, uint16_t reg_address,
		uint8_t *p_values, uint32_t size) {
	return (write_multi(p_platform->fd, p_platform->address, reg_address,
			p_values, size));
}
...
uint8_t wait_for_dataready(VL53L5CX_Platform *p_platform) {
	...
	if (ioctl(p_platform->fd, ST_TOF_IOCTL_WAIT_FOR_INTERRUPT) < 0)
		return 0;
#else
	VL53L5CX_Configuration *p_dev = (VL53L5CX_Configuration*) (p_platform
			- offsetof(VL53L5CX_Configuration, platform));
	uint8_t isReady = 0;
	do {
		WaitMs(p_platform, 5);
