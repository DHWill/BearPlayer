...
static uint8_t _vl53l5cx_poll_for_answer(VL53L5CX_Configuration *p_dev,
		uint16_t address, uint8_t expected_value) {
...
static uint8_t _vl53l5cx_program_output_config(VL53L5CX_Configuration *p_dev) {
	...
	uint32_t output_bh_enable[] = { 0x0001FFFFU, 0x00000000U, 0x00000000U,
			0xC0000000U };

	/* Send addresses of possible output */
	uint32_t output[] = { 0x0000000DU, 0x54000040U, 0x9FD800C0U, 0x9FE40140U,
			0x9FF80040U, 0x9FFC0404U, 0xA0FC0100U, 0xA10C0100U, 0xA11C00C0U,
			0xA1280902U, 0xA2480040U, 0xA24C0081U, 0xA2540081U, 0xA25C0081U,
			0xA2640081U, 0xA26C0084U, 0xA28C0082U };
...
uint8_t vl53l5cx_calibrate_xtalk(VL53L5CX_Configuration *p_dev,
		uint16_t reflectance_percent, uint8_t nb_samples,
	...
	if (((reflectance < (uint16_t) 1) || (reflectance > (uint16_t) 99))
		...
		status |= WrMulti(&(p_dev->platform), 0x2c28, p_dev->temp_buffer,
		...
		VL53L5CX_DCI_CAL_CFG, 8, (uint8_t*) &distance, 2, 0x00);

		status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer,
		VL53L5CX_DCI_CAL_CFG, 8, (uint8_t*) &reflectance, 2, 0x02);

		status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer,
		VL53L5CX_DCI_CAL_CFG, 8, (uint8_t*) &samples, 1, 0x04);
		...
		do {
			...
			if (p_dev->temp_buffer[0] != VL53L5CX_STATUS_ERROR) {
				...
				if ((p_dev->temp_buffer[2] >= (uint8_t) 0x7f)
						&& (((uint16_t) (p_dev->temp_buffer[3] & (uint16_t) 0x80)
								>> 7) == (uint16_t) 1)) {
					(void) memcpy(p_dev->xtalk_data, p_dev->default_xtalk,
	...
	status |= WrMulti(&(p_dev->platform), 0x2fb8, p_dev->temp_buffer,
	...
	(void) memcpy(
			&(p_dev->xtalk_data[VL53L5CX_XTALK_BUFFER_SIZE - (uint16_t) 8]),
...
uint8_t vl53l5cx_get_caldata_xtalk(VL53L5CX_Configuration *p_dev,
...
uint8_t vl53l5cx_set_caldata_xtalk(VL53L5CX_Configuration *p_dev,
		uint8_t *p_xtalk_data) {
	uint8_t resolution, status = VL53L5CX_STATUS_OK;
...
uint8_t vl53l5cx_get_xtalk_margin(VL53L5CX_Configuration *p_dev,
...
uint8_t vl53l5cx_set_xtalk_margin(VL53L5CX_Configuration *p_dev,
	...
	if (margin_kcps > (uint32_t) 10000) {
		...
		status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer,
		VL53L5CX_DCI_XTALK_CFG, 16, (uint8_t*) &margin_kcps, 4, 0x00);
	}
