...
uint8_t vl53l5cx_start_ranging(VL53L5CX_Configuration *p_dev) {
	...
	status |= WrMulti(&(p_dev->platform),
			VL53L5CX_UI_CMD_END - (uint16_t) (4 - 1), (uint8_t*) cmd,
			sizeof(cmd));
	status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1,
	VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53l5cx_get_ranging_data(VL53L5CX_Configuration *p_dev,
	...
	for (i = (uint32_t) 16; i < (uint32_t) p_dev->data_read_size; i +=
		...
		case VL53L5CX_METADATA_IDX:
			p_results->silicon_temp_degc = (int8_t) p_dev->temp_buffer[i
					+ (uint32_t) 12];
			break;
...
uint8_t vl53l5cx_enable_internal_cp(VL53L5CX_Configuration *p_dev) {
	...
	status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer,
	VL53L5CX_DCI_INTERNAL_CP, 16, (uint8_t*) &vcsel_bootup_fsm, 1, 0x0A);

	status |= vl53l5cx_dci_replace_data(p_dev, p_dev->temp_buffer,
	VL53L5CX_DCI_INTERNAL_CP, 16, (uint8_t*) &analog_dynamic_pad_0, 1, 0x0E);
...
uint8_t vl53l5cx_get_VHV_repeat_count(VL53L5CX_Configuration *p_dev,
...
uint8_t vl53l5cx_set_VHV_repeat_count(VL53L5CX_Configuration *p_dev,
